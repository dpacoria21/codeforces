...
void solve() {
    int n; cin>>n;
    map<ll,vl>ma, mb;
    vl as(n);each(a,as)cin>>a;
    vl bs(n);each(a,bs)cin>>a;
    for(int i = 0; i<n; i++) {
        ma[as[i]].pb(i);
        mb[bs[i]].pb(i);
    }
    ...
    for(ll i = n-1; i>=0; i--) {
        ma[as[i]].pop_back();
        ll pos1 = -1, pos2 = -1;
        if(!ma[as[i]].empty()) pos1 = ma[as[i]].back();
        if(!mb[as[i]].empty()) pos2 = mb[as[i]].back();
        if(i-pos2==1) pos2 = -1;
        if(pos1!=-1) {
            ans = max(ans, pos1+1);
        }
        ...
        mb[bs[i]].pop_back();
        pos1 = -1, pos2 = -1;
        if(!ma[bs[i]].empty()) pos1 = ma[bs[i]].back();
        if(!mb[bs[i]].empty()) pos2 = mb[bs[i]].back();
        if(i-pos1==1) pos1 = -1;
        if(pos1!=-1) {
            ans = max(ans, pos1+1);
        }
