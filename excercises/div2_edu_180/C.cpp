...
void solve() {
    ...
    sor(arr);
    ll ans = 0;
    auto isPossible = [&](ll a, ll b, ll c, ll d) {
    ...
    for(ll i = 1; i<n-1; i++) {
        for(ll j = n; j>i+1; j--) {
            ll lim = 1;
            if(j!=n) lim = arr[n]-(arr[i]+arr[j])+1; 
            ll mn = max({arr[i], lim, max(arr[i],arr[j])-min(arr[i],arr[j])+1});
            ll lo = lower_bound(arr.begin()+i, arr.end(), mn) - arr.begin();
            ll mx = min({arr[i]+arr[j]+1, arr[j-1]});
            ll hi = upper_bound(arr.begin()+i, arr.end(), mx) - arr.begin();
            if(hi>=j) hi = j-1;
            if(lo<=i) lo = i+1;
            if(hi==lo) {
                if(j!=n) lim = arr[n];
                else lim = 0;
                if(isPossible(arr[i],arr[j],arr[lo],lim)) ans+=(hi-lo+1);
            }else {
                if((hi-lo+1)>=0) ans+=(hi-lo+1);
            }
        }
    }
    cout << ans << "\n";
