...
void solve() {
    str s, t; cin>>s>>t;
    vector<pair<char,ll>> scount, tcount;
    s.pb('W'); t.pb('W');
    auto count = [&](str a, vector<pair<char,ll>> &arr) {
        ll cnt = 1;
        char curr = a[0];
    ...
    count(s, scount);
    count(t, tcount);
    if(tcount.size()>scount.size() || tcount.size()<scount.size()) {
        cout << "NO\n";
        return;
    }
    for(int i = 0; i<scount.size(); i++) {
        if(scount[i].f==tcount[i].f && (tcount[i].s<=(scount[i].s*2) && tcount[i].s>=scount[i].s)) {
        }else {
            cout << "NO\n";
            return;
        }
