...
void solve() {
    ...
    multiset<ll>mn;
    mn.insert(1e9);
    each(a,arr) mx.insert(a);
    str ans;
    for(int i = 0; i<n; i++) {
        safeErase(mx, arr[i]);
        mn.insert(arr[i]);
        ll valMn = (*mn.begin()), valMx = (*(--mx.end()));
        mx.insert(arr[i]);
        safeErase(mn, arr[i]);
        ll valMn2 = (*mn.begin()), valMx2 = (*(--mx.end()));
        // dbg(valMn, valMx, valMn2, valMx2);
        if((valMn == arr[i] || valMx==arr[i] || valMn2==arr[i] || valMx2 == arr[i])) {
            ans.pb('1');
        }else {
            ans.pb('0');
        }
