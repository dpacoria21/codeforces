...
void solve(){
    ...
    auto getValues = [&](db r, db x, db xi) {
        return ((ll)(sqrtl(r*r - (x-xi)*(x-xi))))*2+1;
    };

    for(int i=0; i<n; i++) {
        ll lo = circs[i].f-circs[i].s, hi = circs[i].f+circs[i].s;
        for(int j = lo; j<=hi; j++) {
            dbg(getValues(circs[i].s, j, circs[i].f));
            mm[j] = max(mm[j], getValues(circs[i].s, j, circs[i].f));
        }
    }
