...
void solve() {
    ...
    while(q--) {
        ...
        if(s=="->") {
            ...
            ll ans = 0;
            while(!m.count({x, y})) {
                ll pos = binpow(2LL, aux-1);
                ll px = cdiv(x, pos);
                ll py = cdiv(y, pos);
                if(px==1 && py==2) {
                    ans+=(binpow(2LL, 2*aux-2)*3);
                }else if(px==2&&py==1) {
                    ans+=(binpow(2LL, 2*aux-2)*2);
                }else if(px==2&&py==2) {
                    ans+=(binpow(2LL, 2*aux-2));
                }
                if(px>1) x-=pos;
                if(py>1) y-=pos;
                aux--;
            ...
            while(!minv.count(x)) {
                ll pos = binpow(2LL, aux-1);
                ll val = binpow(2LL, 2*aux-2);
                if(x>val && x<=(val*2)) {
                    p1+=pos;
                    p2+=pos;
                    x-=(val);
                }else if(x>(val*2) && x<=(val*3)) {
                    p1+=pos;
                    x-=(val*2);
                }else if(x>(val*3) && x<=(val*4)) {
                    p2+=pos;
                    x-=(val*3);
                }
                aux--;
            }
            cout << p1+minv[x].f << " " << p2+minv[x].s << "\n";
        }
