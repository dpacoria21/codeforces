...
void solve() {
    ...
    sort(all(arr), [&](auto a, auto b, auto c, auto d) {
        return a.f.f>b.f.f+a.f.s>b.f.s;
    });
    for(int i = 0; i<n/2; i++) {
        safeErase(xs, (pl){arr[i].f.f, arr[i].s});
        safeErase(ys, (pl){arr[i].f.s, arr[i].s});
        ll sum1 = 0, sum2 = 0, sum3=0, sum4 = 0;
        pl xLo = *(xs.begin());
        sum1 = abs(m[xLo.s].f-arr[i].f.f)+abs(arr[i].f.s-m[xLo.s].s);
        pl xUp = *(--xs.end());
        sum2 = abs(m[xUp.s].f-arr[i].f.f)+abs(arr[i].f.s-m[xUp.s].s);
        pl yLo = *(ys.begin());
        sum3 = abs(m[yLo.s].f-arr[i].f.f)+abs(arr[i].f.s-m[yLo.s].s);
        pl yUp = *(--ys.end());
        sum4 = abs(m[yUp.s].f-arr[i].f.f)+abs(arr[i].f.s-m[yUp.s].s);
        ll mx = max({sum1,sum2,sum3,sum4});
        if(mx==sum1) {
            cout << arr[i].s+1 << " " << xLo.s+1 << "\n";
            safeErase(xs, (pl){m[xLo.s].f, xLo.s});
            safeErase(ys, (pl){m[xLo.s].s, xLo.s});
        }else if(mx==sum2) {
            cout << arr[i].s+1 << " " << xUp.s+1 << "\n";
            safeErase(xs, (pl){m[xUp.s].f, xUp.s});
            safeErase(ys, (pl){m[xUp.s].s, xUp.s});
            
        }else if(mx == sum3) {
            cout << arr[i].s+1 << " " << yLo.s+1 << "\n";
            safeErase(xs, (pl){m[yLo.s].f, yLo.s});
            safeErase(ys, (pl){m[yLo.s].s, yLo.s});
        }else {
            cout << arr[i].s+1 << " " << yUp.s+1 << "\n";
            safeErase(xs, (pl){m[yUp.s].f, yUp.s});
            safeErase(ys, (pl){m[yUp.s].s, yUp.s});
        }
    }   
}
