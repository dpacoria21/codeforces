...
void solve() {
    ...
    if(n==1) { cout << 1 << "\n"; return;}
    auto getArea = [&]() -> pl{
        ll x = (*(--monsX.end())).f - (*monsX.begin()).f + 1;
        ll y = (*(--monsY.end())).f - (*monsY.begin()).f + 1;
        return {x, y};
    };
    pl currDims = getArea();
    ll ans = (currDims.f)*(currDims.s);
    dbg(monsX);
    dbg(monsY);
    dbg(mons);
    dbg(currDims, ans);
    for(int i = 0; i<n; i++) {
        pl a = mons[i];
        monsX.erase(monsX.find(a));
        monsY.erase(monsY.find({a.s, a.f}));
        pl dims = getArea();
        ll currArea = dims.f*dims.s;
        if(n<=currArea) {
            ans = min(ans, currArea);
        }else {
            ans = min(ans, (dims.f+1)*(dims.s));
            ans = min(ans, (dims.f)*(dims.s+1));
        }
        monsX.insert(a);
        monsY.insert({a.s, a.f});
    }
    cout << ans << "\n";
}
