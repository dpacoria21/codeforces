...
void solve() {
    ...
    if(n<=3) {
        ...
        ll ans = (( (s.find("0101")!=str::npos) || (s.find("1010")!=str::npos) ) ? 4 : 3);
        vector<pair<char,ll>> qts;
        ll cnt = 1;
        char curr = s[0];
        for(int i = 1; i<n; i++) {
            if(s[i]!=curr) {
        ...
        qts.pb({curr, 0});
        qts.pb({curr, 0});
        for(int i = 0; i<qts.size()-2; i++) {
            pl l = qts[i], m = qts[i+1], r = qts[i+2];

            ans = max({ans, l.s, m.s, r.s});

            ans = max({ans, l.s+min(m.s, 1LL)});
            ans = max({ans, m.s+min(l.s, 1LL)});

            ans = max({ans, m.s+min(r.s, 1LL)});
            ans = max({ans, r.s+min(m.s, 1LL)});
            
            if(l.f=='1' && m.f=='0' && r.f=='1' && m.s==1) {
                ans = max(ans, l.s+m.s+r.s);
            }else if(l.f=='0' && m.f=='1' && r.f=='0' && m.s==1) {
                ans = max(ans, l.s+m.s+r.s);
            }
        }
        cout << ans << "\n";
