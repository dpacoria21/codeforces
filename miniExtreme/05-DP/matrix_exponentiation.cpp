...
vvl mtx_mult (vvl A, vvl B) {
    pl n = {A.size(), A[0].size()}, m = {B.size(), B[0].size()};
    vvl C(n.f, vl(m.s));
    for(int i = 0; i<n.f; i++) {
        for(int j = 0; j<n.s; j++) {
            for(int k = 0; k<m.f; k++) {
...
void solve() {
    vvl A = {{1, 4, 7}, {2, 5, 8}, {3, 6, 9}};
    vvl B = {{1, -1, 2}, {2, -1, 2}, {3, -3, 0}};
    vvl C1 = mtx_mult(A,mtx_mult(A,A));
    vvl C2 = mtx_exp(A, 3);
    dbg(C1);
    dbg(C2);
}
