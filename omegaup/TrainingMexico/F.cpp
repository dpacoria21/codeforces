...
void solve() {
    ...
    vector<lll>ans;
    for(ll i = cdiv(H+cdiv(G,Pl),hl); i>=0; i--) {
        ll j = cdiv(max(0LL, H-i*hl), hs);
        if(i>=0 && j>=0) ans.emplace_back(i, j, i*Pl+j*ps+(i*Pl+j*ps>=G ? 0:E));
    }
    for(ll i = cdiv(H+cdiv(G,ps),hs); i>=0; i--) {
        ll j = cdiv(max(0LL, H-i*hs), hl);
        if(i>=0 && j>=0) ans.emplace_back(j, i, i*ps+j*Pl+(i*ps+j*Pl>=G ? 0:E));
    }
    sor(ans);
    ll mn = LLONG_MAX;
