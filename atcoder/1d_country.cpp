...
void solve(){
    ll n; cin>>n;
    vl x(n), p(n); each(a,x)cin>>a; each(a,p)cin>>a;
    vl pref(1,0);
    for(int i = 0; i<n; i++) {
        pref.pb(pref.back()+p[i]);
    }
    ll q; cin>>q;
    while(q--) {
        ll l,r; cin>>l>>r;
        ll posL = lower_bound(all(x), l) - x.begin();
        ll posR = upper_bound(all(x), r) - x.begin();
        ll ans = ((pref[posR]) - (pref[posL]));
        cout << ans << "\n";
    }
}
