...
void solve(){
    ...
    for(int i = path.size()-1; i>=0; i--) {
        if(!edges.count({path[i], y})){
            path.pop_back();
        }else {
            break;
        }
    }
    while(path.size()>=2 && edges.count({path[path.size()-1], y}) && edges.count({path[path.size()-2], y})) {
        if(path[path.size()-1]>y) path.pop_back();
        else break;
    }
