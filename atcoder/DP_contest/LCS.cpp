...
void solve() {
    str s, t; cin>>s>>t;
    function<ll(ll,ll)>rcv_solve = [&](ll i, ll j) {
        if(i==s.size()) return 0LL;
        if(j==t.size()) return 0LL;
        ll ans = 0, ans1 = 0, ans2 = 0;
        if(s[i]==t[j]) {
            ans = rcv_solve(i+1, j+1) + 1;
        }else {
            ans1 = rcv_solve(i+1, j) + (s[i]==t[j]);
            ans2 = rcv_solve(i, j+1) + (s[i]==t[j]);
        }
        return max({ans1,ans2,ans});
    };

    vvl dp(s.size(), vl(t.size(), -1));
    vvl path(s.size(), vl(t.size(), 0));
    function<ll(ll,ll)>dp_solve = [&](ll i, ll j) {
        if(i==s.size()) return 0LL;
        if(j==t.size()) return 0LL;
        if(dp[i][j]!=-1) return dp[i][j];
        ll ans = 0, ans1 = 0, ans2 = 0;
        if(s[i]==t[j]) {
            ans = dp_solve(i+1, j+1) + 1;
            path[i][j] = 1;
        }else {
            ans1 = dp_solve(i+1, j);
            ans2 = dp_solve(i, j+1);
            if(ans1>=ans2) {
                path[i][j] = 2; // hacia abajo i+1
            }else {
                path[i][j] = 3; // hacia la derecha j+1
            }
        }
        return dp[i][j] = max({ans1,ans2,ans});
    };
    dp_solve(0,0);
    str ans = "";
    ll i = 0, j = 0;
